...
import { EMERGENCY_RE, CATEGORY_MAP } from '@/lib/helpers';
...
export default function CreatePage() {
    ...
    const [category, setCategory] = useState({ label: 'General', cat: 'general', type: 'general' });
    ...
    const detectCategory = (text) => {
        let matched = { label: 'General', cat: 'general' };
        for (const { re, label, cat } of CATEGORY_MAP) {
            if (re.test(text)) { matched = { label, cat }; break; }
        }
        setCategory(matched);
    ...
    const handleSubmit = (e) => {
        ...
        const tags = [{ label: category.label, type: category.label === 'General' ? 'general' : 'specialty' }];
        if (EMERGENCY_RE.test(body)) tags.push({ label: 'URGENT', type: 'urgent' });

        DB.addPost({ title: subject.trim(), body: body.trim(), tags, category: category.cat });
    ...
    const charColor = body.length > charLimit * 0.9 ? 'var(--red-500, #ef4444)' : body.length > charLimit * 0.7 ? 'var(--amber-500, #f59e0b)' : 'var(--primary)';
            ...
            <div className="create-form">
                ...
                <div className="create-guidelines">
                    ...
                    <div className="create-guidelines__content">
                        <h4 className="create-guidelines__title">Writing a great health question</h4>
                ...
                <form id="create-form" onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        <label className="form-label">
                            <span className="material-icons-round" style={{ fontSize: '14px', verticalAlign: 'middle', marginRight: '4px' }}>title</span>
                            Subject
                        </label>
                        <input
                        ...
                        />
                        <span className="form-hint" style={{ marginTop: '0.25rem' }}>A clear title helps professionals respond faster</span>
                    </div>

                    {/* Description */}
                    <div className="form-group">
                        <label className="form-label">
                            <span className="material-icons-round" style={{ fontSize: '14px', verticalAlign: 'middle', marginRight: '4px' }}>edit_note</span>
                            Description
                        </label>
                        <div className="textarea-wrap">
                            ...
                            <div className="textarea-footer">
                                <div className="textarea-tools">
                                    <button type="button" className="icon-btn icon-btn--xs" title="Bold">
                                        <span className="material-icons-round">format_bold</span>
                                    </button>
                                    <button type="button" className="icon-btn icon-btn--xs" title="List">
                                        <span className="material-icons-round">format_list_bulleted</span>
                                    </button>
                                    <button type="button" className="icon-btn icon-btn--xs" title="Attach">
                                        <span className="material-icons-round">attach_file</span>
                                    </button>
                                </div>
                                <div className="char-count-ring">
                                    <svg width="24" height="24" viewBox="0 0 24 24">
                                        <circle cx="12" cy="12" r="10" fill="none" stroke="var(--slate-200)" strokeWidth="2.5" />
                                        <circle cx="12" cy="12" r="10" fill="none" stroke={charColor} strokeWidth="2.5"
                                    ...
                                    <span className="char-count" style={{ color: charColor }}>{body.length}</span>
                                </div>
                            </div>
                            {showTriage && (
                    ...
                    {showCategory && (
                        <div className="category-preview" id="category-preview">
                            ...
                            <div className="category-preview__content">
                                <span className="category-preview__label">AI-Detected Category</span>
                                <span className={`tag ${category.label === 'General' ? 'tag--general' : 'tag--specialty'}`} id="category-tag">
                                    {category.label}
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="create-extras">
                        <label className="extra-check">
                            ...
                            <span className="material-icons-round" style={{ fontSize: '18px', color: 'var(--slate-400)' }}>
                        ...
                        <label className="extra-check">
                            ...
                            <span className="material-icons-round" style={{ fontSize: '18px', color: 'var(--slate-400)' }}>notifications</span>
                    ...
                    <div className="create-submit-area">
                        <div className="create-submit-area__info">
                            ...
                            <span>Posts are reviewed by AI for category triage. Verified professionals may respond within 24 hours.</span>
                        </div>
                        <button
                            type="submit"
